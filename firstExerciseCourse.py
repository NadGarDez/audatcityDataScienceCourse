...
def avg_medal_count():
    ...
    b = numpy.array(df[["gold","silver","bronze"]][(df["gold"]>0) & (df["silver"]>0) & (df["bronze"]>0)])
    p = numpy.array(df["country_name"][(df["gold"]>0) & (df["silver"]>0) & (df["bronze"]>0)])

    c = []

    for i in b:
        c.append(numpy.mean(i))

    serie = Series(c,index=p)
